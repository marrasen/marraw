...
- Masks: a **Background** button next to Subject — one click detects the subject, inverts the mask so it selects everything else, and defocuses it with a bloom and light streaks. Tune it from there, or drop it into a preset and apply the whole look to a shoot
