...
const (
	...
	// burstMaxHamming is the dHash distance (of 64 bits) up to which two
	// adjacent frames count as the same composition. 10 is the classic
	// near-duplicate cutoff; recomposed shots land well above it.
	burstMaxHamming = 10
)
...
// perceptually similar. Only groups of two or more get an id — the first
// member's photo ID, so ids are stable across refreshes as long as the
// group's lead frame stays. Untimed (taken_at = 0) and unhashed photos
// never group.
func burstGroups(photos []store.Photo) map[int64]int64 {
	groups := make(map[int64]int64)
	start := 0 // index of the current chain's first member
	for i := 1; i <= len(photos); i++ {
		if i < len(photos) && linked(photos[i-1], photos[i]) {
			continue
		}
		if i-start >= 2 {
			id := photos[start].ID
...
func linked(a, b store.Photo) bool {
	...
	return pyramid.HammingDist(uint64(a.PHash.Int64), uint64(b.PHash.Int64)) <= burstMaxHamming
}
