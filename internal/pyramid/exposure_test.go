...
func TestRenderPreviewExposureDelta(t *testing.T) {
	...
	base := meanLuma(RenderPreview(src, 64, 0.72, e, 0, nil, nil))
	up := meanLuma(RenderPreview(src, 64, 0.72, e, 1, nil, nil))
	down := meanLuma(RenderPreview(src, 64, 0.72, e, -1, nil, nil))
	...
	if got := meanLuma(RenderPreview(src, 64, 0.72, e, 0, nil, nil)); math.Abs(got-meanLuma(noFold)) > 0.5 {
