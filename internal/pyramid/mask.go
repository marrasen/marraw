...
func ApplyMasks(img *image.RGBA, e *edit.Params) {
	if !e.HasMasks() {
		return
	}
	bnd := img.Bounds()
...
func newMaskEvaluator(m *edit.Mask, f maskFrame) maskEvaluator {
...
func (ev *brushEval) strokeBounds(strokes []edit.Stroke) {
	...
	// The frame→output map may rotate; bound the box by its corners.
