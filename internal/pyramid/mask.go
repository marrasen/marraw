...
// neighbouring pixels — so they run as a separate LATER pass per mask
// (applyMaskFX), after the tone LUTs: "push the background −1 EV AND streak it"
// darkens the region first and adds the streak light on top, so a highlight
// stays white instead of the dimmed grey it would be if exposure came last.
// That matches the global pipeline, where exposure precedes everything. A mask
// without FX never enters that pass, so FX-free renders stay byte-identical.
//
...
func ApplyMasks(img *image.RGBA, e *edit.Params, ai AIMapSet) []uint8 {
	...
	for mi := range e.Masks {
		...
		// Tone first, FX second: the spatial effects must gather (and add light
		// to) already-toned pixels, or this mask's own exposure would darken the
		// light its streaks and glow just added.
		if m.Adjust.HasTone() { // an FX-only mask's LUTs would be the identity
			if wrow == nil {
				wrow = make([]uint16, w)
			}
...
func newAIEval(m *edit.Mask, f maskFrame, ai AIMapSet) *brushEval {
	...
	// A background mask IS the inverted subject matte, so its kind and the
	// Invert pill compose: inverting a Background mask selects the subject.
	invert := m.Invert != (m.AIKind == edit.AIBackground)
	ev := &brushEval{
