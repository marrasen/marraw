...
- **Disc bokeh for the mask blur.** Done 2026-08-09: a separate Bokeh dial in
  the mask Effects block — a hard-edged disc gather in O(r) per pixel via
  per-row prefix sums (a disc is a stack of spans), weight-graded like Blur,
  with the gather favouring bright pixels in linear light so a defocused
  point light stays a bright disc (`internal/pyramid/gradedblur.go`).
