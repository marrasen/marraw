...
export interface AIMapsGeneratedEvent {
    photoId: number;
}
...
export function generateAIMaps(client: ApiClient, photoIDs: number[], kinds: AIKindType[], allowDownload: boolean, options?: RequestOptions): Promise<TaskRef> {
    return client.request<TaskRef>('Edits.GenerateAIMaps', [photoIDs, kinds, allowDownload], options);
}
// Wire-method tag consumed by useQuerySuspense to key the promise cache and
// open the matching server subscription. Stable identifier across builds
// (unaffected by minification, unlike Function.name).
generateAIMaps.method = 'Edits.GenerateAIMaps' as const;
...
export function onAIMapsGeneratedEvent(client: ApiClient, handler: PushHandler<AIMapsGeneratedEvent>): () => void {
    return client.onPush<AIMapsGeneratedEvent>('AIMapsGeneratedEvent', handler);
}
...
/**
 * Subscribes to `Edits.GenerateAIMaps` with the given parameters and re-renders
 * automatically when the server triggers a refresh. When the parameters
 * change, the previous subscription is canceled and a new one starts.
 * See {@link UseQueryResult} for return value details — including the
 * query-scoped `mutate(action)` helper for refetch-after-mutation flows.
 */
export function useGenerateAIMaps(photoIDs: number[], kinds: AIKindType[], allowDownload: boolean, options?: UseQueryOptions<TaskRef>): UseQueryResult<TaskRef> {
    const wrappedFn = useCallback(
        (client: ApiClient, signal: AbortSignal, photoIDs: number[], kinds: AIKindType[], allowDownload: boolean) => generateAIMaps(client, photoIDs, kinds, allowDownload, { signal }),
        [],
    );
...
export function useAIMapsGeneratedEvent(options?: { maxEvents?: number }): UsePushResult<AIMapsGeneratedEvent> {
    return usePushEvent(onAIMapsGeneratedEvent, options);
}
